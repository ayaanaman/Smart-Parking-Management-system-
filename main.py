...
conn = mysql.connector.connect(
    host='localhost', database=' parking_system', user='aman', password='root')
cursor = conn.cursor()
...
def made_by():
    msg = ''' 
            Parking Management system made by           : Aman Yadav, Aman Ahlawat
            Roll No                                     : 2K19/IT/14, 2K19/IT/12
            semester                                    : 4th
...
def add_new_vehicle():
    ...
    sql = 'select id from parking_space where type_id = {} AND status = "open"'.format(type_id)
    cursor.execute(sql)
